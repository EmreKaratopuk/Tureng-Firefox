...
function tureng(str) {
  ...
  $.get({
    ...
    success: (data) => {
      ...
      if (
        $(data).find("ul.suggestion-list li").length > 0 &&
        ...
        $(data)
          .find("ul.suggestion-list li")
          .each((i, el) => {
            $("#content .list-group").append(`
            <a href="#" class="list-group-item list-group-item-action">${safeResponse.cleanDomString(
              el.textContent.trim()
            )}</a>
          `);
          });
        ...
        $.each(checkSearchResults, function () {
              ...
              <thead class="thead-default">
                <tr>
                  ...
                  <th>${safeResponse.cleanDomString(
                    $(eachRow[0]).find(".c2").text()
                  )}</th>
                  <th>${safeResponse.cleanDomString(
                    $(eachRow[0]).find(".c3").text()
                  )}</th>
                  </tr>
              </thead>
              <tbody>
          ...
          array.forEach((el) => {
            ...
            if (
              ...
              const $type =
                $(el).children("td").eq(2).find("i").text().trim() != ""
                  ? $(el).children("td").eq(2).find("i").text().trim()
                  : "";
              const $definition = $(el)
                .children("td")
                ...
                .text()
                .trim();
              const $definitionType = $(el)
                .children("td")
                .eq(3)
                .find("i")
                .text()
                .trim()
                ? $(el).children("td").eq(3).find("i").eq(0).text().trim()
                : "";
          ...
          <tr>
            ...
            <td><a data-href="${safeResponse.cleanDomString(
              e.word
            )}">${safeResponse.cleanDomString(e.word)}</a> ${
              safeResponse.cleanDomString(e.type) != ""
                ? "<small>(" + safeResponse.cleanDomString(e.type) + ")</small>"
                : ""
            }</td>
            <td><a data-href="${safeResponse.cleanDomString(
              e.definition
            )}">${safeResponse.cleanDomString(e.definition)}</a> ${
              safeResponse.cleanDomString(e.definitionType) != ""
                ? "<small>(" +
                  safeResponse.cleanDomString(e.definitionType) +
                  ")</small>"
                : ""
